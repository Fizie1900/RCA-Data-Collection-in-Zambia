...
def submit_final(interview_id):
    ...
    try:
        ...
        # Set the flag to show completion actions
        ...
        *If you could advise the government on specific, actionable reforms 
        ...
        if submit_btn:
            ...
            if interview_id and submit_final(interview_id):
                ...
                # Set flag to show completion actions
                st.session_state.interview_submitted = True
                st.rerun()
    
    # Show completion actions if interview was just submitted
    if st.session_state.get('interview_submitted', False):
        show_completion_actions()

def show_completion_actions():
    """Show actions after interview completion - MOVED OUTSIDE FORM"""
...
def generate_interview_summary():
    ...
    try:
        if st.session_state.current_interview_id:
            interview_details = get_interview_details(st.session_state.current_interview_id)
            if not interview_details.empty:
                interview = interview_details.iloc[0]
                
                # Create summary data
                summary_data = {
                    "Interview ID": interview['interview_id'],
                    "Business Name": interview['business_name'],
                    "District": interview['district'],
                    "Sector": interview['primary_sector'],
                    "Total Compliance Cost": f"ZMW {interview['total_compliance_cost']:,.0f}",
                    "Total Compliance Time": f"{interview['total_compliance_time']} days",
                    "Risk Score": f"{interview['risk_score']:.1f}/10",
                    "Submission Date": interview['submission_date']
                }
                ...
                st.warning("No interview data found for summary.")
        else:
            st.warning("No active interview to generate summary from.")
    except Exception as e:
